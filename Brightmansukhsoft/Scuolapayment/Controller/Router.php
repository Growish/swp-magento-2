...
class Router implements \Magento\Framework\App\RouterInterface
{
    ...
    public function __construct(\Magento\Framework\App\ActionFactory $actionFactory, \Magento\Framework\Event\ManagerInterface $eventManager, \Magento\Framework\UrlInterface $url, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Framework\App\ResponseInterface $response)
    {
    ...
    public function match(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        if (($uri_segmentsCount == '2') && (strpos($identifier, 'redirect') !== false))
        {
            ...
            $request->setModuleName('scuolapay')
                ->setControllerName('redirect')
                ->setActionName('scuolapayredirect')
                ->setParam('identifier', $param);
        ...
        else if (($uri_segmentsCount == '2') && (strpos($identifier, 'response') !== false))
        {
            ...
            $param = $uri_segments[1];
            $request->setModuleName('scuolapay')
                ->setControllerName('scuolaresponse')
                ->setActionName('response')
                ->setParam('identifier', $param);
            $request->setAlias(\Magento\Framework\Url::REWRITE_REQUEST_PATH_ALIAS, $identifier);
        }
        else
